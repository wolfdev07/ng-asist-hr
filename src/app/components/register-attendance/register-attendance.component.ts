...
export class RegisterAttendanceComponent {
  ...
  sendData() {
    if (this.email.valid && this.employeeNumber.valid) {
      ...
      //http://127.0.0.1:8000/
      this.http.post('https://backend-hr-w7ws2dwj5a-ew.a.run.app/asistApi/lookup/', datos)
        .subscribe(
          response => {
  ...
  registerAttendance(datos: any, photo: File ) {
    ...
    this.http.post('https://backend-hr-w7ws2dwj5a-ew.a.run.app/asistApi/attendance/', formData)
      .subscribe(
        response => {
          console.log('Respuesta del servidor:', response);
